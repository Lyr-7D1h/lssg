...
<!--TODO insert image/video of full built plotter-->
...
- (1-2) Springs (from a ballpoint pen)  
...
- (4m+) 24 AWG wire  
  Stranded silicone wires are recommended because they are less prone to cracking with frequent movement   
  https://aliexpress.com/item/1005005450660428.html
- (10, 6 optional) Female breadboard wire ends
- (10, 6 optional) Heat shrink tubing for the breadboard wires
...
- (2) Hinge Roller Limit switches (optional)  
...
You can download all parts here TODO INSERT. Or go to [thingiverse](TODO). I've printed all my parts on the Bambu P1S. I'm sharing the exact settings I used to print the parts (in the 3mf files).

## Assembly

<!--TODO add pictures-->
<!--TODO review all steps see what makes the most sense-->

### X-axis

### Bottom clamshell

- Grab **bottom clamshell**, **(4) Idler pulleys**, **(4) 624zz bearings**, **(4) M4x35mm screws** and **(4) LM8UU bearings**.
- Push the 3d printed idler pulleys around the 624zz bearings
- Push the **LM8UU bearings** into the **bottom clamshell**
- Screw the 624zz bearings into the 4 spots onto the bottom clamshell
...
- Grab **12V Fan 40mm x 40mm**, **Buck converter 12V - 5V 1A+**, **(6) M3x5mm screws**, **(4) M3x14mm screws**, **(12) M3 nuts**, **(2) M2.5x5mm screws**.
- Install the fan with the air flow going in using the **M3x14mm screws**.
![Fans](./how_to_build_last_draw/fan*.jpg)
- 

### X-axis clamshell

### Assemble the clamshell

- Grab **(4) 624zz bearings**, **Limit switch**, **(2) M2x10mm screws**, **(4) M4x35mm screws**,  and **(8) LM8UU bearings**.
...
- Use the **(4) 624zz bearings** and push them through the 3D printed idler pulleys. 
...
You may need to use a round file to smooth out the holes that you insert them in. Also, you can use a rubber mallet to help insert the rods. 
- Take the **threaded rod** and insert it in the hole below. Feed an M8 washer
and an M8 nut onto both sides of the x-support part.
...
Use a rubber mallet again to attach the last X-support onto the linear rods
...
- Take the **(2) 350mm linear rods** and insert them into the Y-back piece by using a rubber mallet
- Get **(1) M4-0.5 x 35 screws**, **(1) M4 nut** and the **5th 624zz bearing**
...
- Slide the linear rods/Y-back piece through the **LM8UU bearings** and attach the Y-front piece using a rubber mallet
...
- Get **(2) 3mm linear rods** and the following 3D printed parts:
  <!--TODO: Use exact part names from model files-->
  - Slider
  - Pen Holder
  - Base Slider
...
- Get **(3) M3-0.5 x 16mm screws** which you will use to secure the Base Slide to the Y-Front part. You may need to use **(3) M3-0.5 nuts** in order to hold it in place
...
This part explains how to wire all electronics up. See the [DLC32 manufacturer's wiring manual](./how_to_build_last_draw/dlc32_wiring_manual.pdf) for more information.

### Stepper Motor Drivers
> [!WARNING]
> Don't plug or unplug the motor and driver when the board is running to avoid malfunction.

Time to install the **A4988** stepper motor drivers. 

1. Ensure that all three pins are set to ON for both driver slots so it's set to 1/16 step mode. 

![driver slot](./how_to_build_last_draw/micro_step.png)

2. Install the drivers in the X-axis and Y-axis **Motor drive slots**. Using the orientation of the wiring above make sure to orient the drivers so that the pins match the following image. Where the `EN` pin is on the green side.
...
Solder long enough wires on both limit switches. Solder them onto the pins **NO** and **C**. 

Our mechanical switches only need **GND** and **S** wires to work. **GND** should match **C** on the limit switch and **S** should match **NO**. Wire the limit switch placed next to the stepper motor to the **X axis limit switch** and the limit switch on the clamshell to the **Y axis limit switch** pinout.

![limit switch](./how_to_build_last_draw/limit_switch.png)
...
This button is used to reset the [MCU](https://en.wikipedia.org/wiki/Microcontroller) or as an emergency stop for the device.

Solder two breadboard wires to the two pins opposite of each other. Connect the two wires to the **Runback button interface**.

## Firmware
> [!WARNING] 
> Makerbase DLC32 uses a CH340 USB chip, make sure you have the needed drivers installed on your machine before interacting with the board.

We're gonna use [FluidNC](http://wiki.fluidnc.com/en/home) which is a popular [CNC](https://en.wikipedia.org/wiki/Computer_numerical_control) firmware. It's an actively maintained and widely supported project. We're going to follow some of the configuration from the [documented setup for the Makerbase MKS DLC32](http://wiki.fluidnc.com/en/hardware/3rd-party/MKS_DLC32).


See the [FluidNC Installation guide](http://wiki.fluidnc.com/en/installation) for installing the firmware on the board.

Use the following config to configure the board
