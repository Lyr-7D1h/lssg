...
"#;
        ...
        assert_eq!(tokens.len(), 1);
        if let Token::Table { header, rows, .. } = &tokens[0] {
            assert_eq!(header.len(), 4); // 4 columns
            assert_eq!(rows.len(), 3); // 3 data rows

            // Check that code blocks are preserved in cells
            if let Some(Token::Text { text }) = rows[0][0].first() {
                assert!(text.contains("`footer.items`"));
            } else {
                panic!("Expected text token in first cell");
            }

            if let Some(Token::Text { text }) = rows[0][2].first() {
                assert!(text.contains("`[]`"));
            } else {
                panic!("Expected text token in default cell");
            }
        } else {
            panic!("Expected table token, got {:?}", tokens[0]);
        }
